    ...
    render(){
        ...
        for (var i = 0; i < this.props.numberOfMessages; i++){
            ...
            if (this.props.moveMessages){
                messageList.push(<li className="indivMessage" key={i}><marquee behaviour="alternate" onClick={() => {window.open(("//" + "google.com/search?q=" + message.toString()), 'popup', 'width=400,height=400' ); return false;}} onContextMenu={() => {window.open(("https://www.youtube.com/results?search_query=" + message.toString()), 'popup', 'width=400,height=400'); return false;}} data-hover="Left click for google, right click for youtube"> {message} </marquee></li>);
            } else {
